...
abstract class AbstractUVCCameraHandler extends Handler {
	...
	static final class CameraThread extends Thread {
		...
		CameraThread(final Class<? extends AbstractUVCCameraHandler> clazz,
					 ...
					 final float bandwidthFactor, OnCaptureListener onCaptureListener) {
			...
			try {
				...
				try (BufferedOutputStream os = new BufferedOutputStream(new FileOutputStream(outputFile))) {
					try {
						...
						os.flush();
						//todo
						OnCaptureListener onCaptureListener=mOnCaptureListener;
						if(onCaptureListener!=null){
							if (outputFile != null) {
								String path1 = outputFile.getPath();
								Log.i(TAG,"Save bitmap finish, path: "+path1);
								onCaptureListener.onCompleteCapture(path1);
							}
						}
						if (outputFile != null) {
